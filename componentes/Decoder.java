...
public class Decoder {
	...
	public void instAdd(){
		...
		regsGroup.regDestino.addLast(regDestino);
		System.out.println(">>> Registrador destino: " +inst[1] );

		// identifica registradores fonte
		int indiceOp1 = Integer.parseInt(inst[2].substring(1));
		int indiceOp2 = Integer.parseInt(inst[3].substring(1));

		String op1 = regsGroup.registers[indiceOp1].data;
		String op2 = regsGroup.registers[indiceOp2].data;
		System.out.println(">>> Primeiro registrador fonte: " +inst[2] );
		System.out.println(">>> Primeiro operando fonte: " + op1);
		System.out.println(">>> Segundo registrador fonte: " +inst[3] );
	...
	public void instLw(){
		...
		// identifica registrador destino
		String regDestinoAt = inst[1];
		regsGroup.regDestino.addLast(regDestinoAt);
		System.out.println(">>> Registrador destino: " +inst[1] );
	...
	public void instSt(){
		...
		// recuperar dado do registrador
		String regFonte = inst[2];
		System.out.println(">>> Registrador Fonte: " +inst[2] );

		// indentificar índice do registrafor
		int endMenFonte = Integer.parseInt(inst[2].substring(1));
		String dadoReg = regsGroup.registers[endMenFonte].data;

		// recuperar onde salvar o dado
		String endDest = inst[1];
		System.out.println(">>> Salvando dado em : " +inst[1] );
		Signal signalDest = new Signal(endDest);
		Signal signalDado = new Signal(dadoReg);

		System.out.println("DadoReg: " + dadoReg);
		// diz que quer enviar algo por esse sinal
		regsGroup.input1.setEvent(false);
		signalDest.setEvent(true);
		signalDado.setEvent(true);
		regsGroup.output6.setSignal(signalDado);
		regsGroup.output7.setSignal(signalDest);
		regsGroup.output6.write();
		regsGroup.output7.write();
